...
// Ambil data awal
$stmt = $mysqli->prepare("SELECT nama, telp, alamat FROM supplier WHERE id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$stmt->bind_result($nama_db, $telp_db, $alamat_db);
if ($stmt->fetch()) {
    // Jika belum POST, isi dari DB
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $mysqli->prepare("UPDATE supplier SET nama = ?, telp = ?, alamat = ? WHERE id = ?");
