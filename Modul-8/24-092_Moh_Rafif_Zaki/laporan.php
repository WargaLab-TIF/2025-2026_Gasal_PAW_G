...
$query = "SELECT stok,nama_barang FROM barang";
$result = mysqli_query($conn, $query);
...
if (isset($_POST['excel'])) {
    header("Location:../fungsi/excel.php?aw=" . $tanggal_aw . "&ak=" . $tanggal_ak);
    exit;
}
$error = [];
...
$query = "SELECT COUNT(t.waktu_transaksi) AS jumlah_transaksi,t.waktu_transaksi FROM transaksi  t GROUP BY t.waktu_transaksi";
$result = mysqli_query($conn, $query);
$jumlah = mysqli_fetch_all($result, MYSQLI_ASSOC);

$waktu_tr = [];
foreach ($jumlah as $j) {
    $s = $j['jumlah_transaksi'];
    $waktu_tr[] = $s;
}
?>
<!DOCTYPE html>
                ...
                <script>
                    ...
                    var myChart = new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                label: "total",
                                data: <?php echo json_encode($waktu_tr); ?>,
