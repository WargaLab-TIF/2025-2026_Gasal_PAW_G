<?php
    ...
    if(isset($_POST['submit'])) {
        ...
        if($error['nama'] == "" && $error['hp'] == "" && $error['alamat'] == "" && $error['username'] == "" && $error['password'] == "") {
            ...
            $level = $_POST['level'];
            $query = $conn->prepare("UPDATE user SET username = ?, password = ?, nama = ?, alamat = ?, hp =? , level = ? WHERE id_user = $id_user");
...
<body>
    ...
    <form action="" method="POST">
        ...
        <div>
            ...
            <span>Nama:</span>
            <input type="text" name="nama" value="<?php echo $_POST['nama'] ?? $hasil['nama'] ?>">
        </div>
        <div>
        ...
        <div>
            ...
            <span>Nomor HP:</span>
            <input type="text" name="hp" value="<?php echo $_POST['hp'] ?? $hasil['hp'] ?>">
        </div>
        <div>
            <span>Jenis User:</span>
            <select name="level" id="level">
                <?php if($_POST['level'] == ""): ?>
                    <?php for ($num=2; $num >=1; $num--): ?> 
                        <?php if($hasil['level'] == $num): ?>
                            <option value="<?php echo $num ?>" selected>
                                <?php
                                    if($num == 1) {
                                        echo "Owner";
                                    } else {
